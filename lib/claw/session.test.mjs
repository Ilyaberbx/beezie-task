...
test("a method without a balance never blocks the order", () => {
  ...
  assert.equal(result.affordableQuantity, MAX_QUANTITY);
  assert.equal(result.methodLabel, "Credit / Debit");
});

test("an unknown method still names itself in the alert", () => {
  assert.equal(afford({ method: undefined }).methodLabel, "this method");
});
...
test("affordableQuantity never exceeds the maximum", () => {
  assert.equal(afford({ method: wallet(100000) }).affordableQuantity, MAX_QUANTITY);
});
